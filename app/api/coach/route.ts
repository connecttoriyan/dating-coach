...
// Receives chat history + userToken + sessionId -> rate-limits ->
// creates/updates Supabase row -> streams Claude response back.
...
const SYSTEM_PROMPT = `You are an AI Dating Coach and Social Coach.
...
export async function POST(req: Request) {
  try {
    ...
    const userToken: string | undefined = body.userToken;
    ...
    if (!userToken) {
      return new Response("Bad request: missing userToken", { status: 400 });
    }

    // ---- Rate limit ----
    const limit = await checkChatLimit(userToken);
    if (!limit.allowed) {
      return Response.json(
        {
          error: "rate_limit",
          message: `You've used today's ${limit.limit} coaching messages. Come back tomorrow — fresh quota then.`,
          used: limit.used,
          limit: limit.limit,
        },
        { status: 429 }
    ...
    // ---- DB: ensure a session row exists ----
    if (!sessionId) {
    ...
    return new Response(`Coach error: ${message}`, { status: 500 });
  }
}
