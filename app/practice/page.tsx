// app/practice/page.tsx — Editorial voice practice
"use client";
...
export default function PracticePage() {
  ...
  async function startCall() {
    ...
    // Pre-check the daily limit so we don't burn a Vapi connection
    // when we'd reject the save afterward anyway.
    try {
      const r = await fetch(
        `/api/voice-limit?userToken=${encodeURIComponent(userTokenRef.current!)}`
      );
      const j = await r.json();
      if (!j.allowed) {
        setLimitMsg(j.message || "Daily practice limit reached.");
        return;
      }
  ...
  async function saveAndGetFeedback() {
    try {
      const saveRes = await fetch("/api/voice-session", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          userToken: userTokenRef.current,
          transcript: turnsRef.current,
        }),
      });
      const saveJson = await saveRes.json();
      if (saveRes.status === 429) {
        setLimitMsg(saveJson.message || "Daily practice limit reached.");
        return;
      }
      if (!saveRes.ok) throw new Error(saveJson.error || "save failed");

      setFeedbackLoading(true);
      const fbRes = await fetch("/api/voice-feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    <div className="min-h-screen bg-cream text-ink flex flex-col">
      ...
      <div className="px-6 md:px-10 pt-10 pb-4">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="font-display text-4xl md:text-5xl">
            A first call with <em className="text-accent">Sarah</em>.
          </h1>
          <p className="text-ink-soft mt-3 text-[15px] max-w-xl">
            Talk like it&apos;s a real first phone call. End it whenever you
            want — the coach will tell you what worked.
          </p>
        </div>
      </div>
      ...
      <section className="px-6 md:px-10 py-12">
        <div className="max-w-3xl mx-auto flex flex-col items-center gap-6">
          ...
          {status === "idle" && !limitMsg && (
            <button
              onClick={startCall}
              className="bg-ink text-cream rounded-full px-8 py-4 text-sm font-medium hover:bg-accent transition mt-2"
            >
              Start the call
            </button>
          )}
          {status === "connecting" && (
            <div className="text-ink-fade text-sm">Dialing…</div>
          )}
          {live && (
            <button
              onClick={endCall}
              className="bg-accent text-cream rounded-full px-8 py-4 text-sm font-medium hover:bg-ink transition"
            >
              End call
            </button>
          )}
          {status === "ended" && (
            <button
              onClick={startCall}
              className="border border-ink/20 hover:border-ink/60 rounded-full px-8 py-4 text-sm font-medium transition"
            >
              Practice again
            </button>
          )}
      ...
      <main className="flex-1 px-6 md:px-10 pb-16">
        <div className="max-w-3xl mx-auto">
          ...
          {error && (
            <div className="border border-accent/30 bg-accent-soft rounded-2xl p-5 mb-8 text-[14px] text-ink">
              {error}
            </div>
          )}
          ...
          {turns.length > 0 && (
            <div className="bg-paper border border-line rounded-2xl p-6 md:p-8 mb-8">
              <div className="text-[10px] tracking-[0.25em] uppercase text-ink-fade mb-5">
                Transcript
              </div>
              <div className="space-y-4">
                {turns.map((t, i) => (
                  <div key={i} className="flex gap-4">
                    <div
                      className={`text-[10px] uppercase tracking-[0.2em] mt-1.5 shrink-0 w-14 ${
                        t.role === "user" ? "text-ink" : "text-accent"
                      }`}
                    >
                      {t.role === "user" ? "You" : "Sarah"}
                    </div>
                    <div className="text-[15px] leading-relaxed text-ink-soft flex-1">
                      {t.text}
                    </div>
                  </div>
                ))}
          ...
          {status === "ended" && (
            <div className="bg-paper border border-line rounded-2xl p-6 md:p-8">
              <div className="text-[10px] tracking-[0.25em] uppercase text-accent mb-5">
                Coach Feedback
              </div>
              {feedbackLoading && (
                <div className="text-ink-fade italic">
                  Coach is listening back…
                </div>
              )}
              {!feedbackLoading && feedback && (
                <div className="font-display text-lg md:text-xl leading-[1.6] text-ink whitespace-pre-wrap">
                  {feedback}
                </div>
              )}
              {!feedbackLoading && !feedback && !error && (
                <div className="text-ink-fade text-sm">
                  No feedback yet — the call may have been too short.
                </div>
              )}
            </div>
          )}
...
function StatusLabel({
  ...
  if (status === "connecting") text = "Dialing…";
    ...
    if (speaker === "sarah") {
      text = "Sarah is talking";
      cls = "text-accent";
    } else if (speaker === "user") {
      text = "She's listening";
      cls = "text-ink";
    } else {
      text = "Live";
      cls = "text-ok";
  ...
  return (
    <div className={`text-[11px] tracking-[0.25em] uppercase ${cls}`}>
      · {text} ·
    </div>
  );
}
