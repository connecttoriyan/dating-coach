// app/sessions/page.tsx — Editorial past-sessions list
"use client";
...
export default function SessionsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/sessions?userToken=${encodeURIComponent(token)}`)
      .then((r) => r.json())
      .then((data) => {
      ...
      <div className="px-6 md:px-10 pt-10 pb-6">
        <div className="max-w-3xl mx-auto">
          ...
          <h1 className="font-display text-4xl md:text-5xl">
            Everything you&apos;ve worked through.
          </h1>
        </div>
      </div>
      ...
      <main className="flex-1 px-6 md:px-10 pb-16">
        <div className="max-w-3xl mx-auto">
          ...
          {!loading && !error && sessions.length === 0 && (
            <Empty>
              Nothing here yet.{" "}
              <Link href="/coach" className="underline hover:text-accent">
          ...
          {!loading && sessions.length > 0 && (
            <div className="space-y-3">
              {sessions.map((s) => {
                ...
                const firstMsg =
                  s.transcript.find((m) => m.role === "user")?.content ?? "(empty)";
                ...
                return (
                  <div
                    key={s.id}
                    className="bg-paper border border-line rounded-2xl overflow-hidden transition hover:border-line-strong"
                  >
                    <button
                    ...
                    >
                      ...
                      <div className="text-[15px] text-ink leading-relaxed line-clamp-2">
                        {firstMsg}
                      </div>
                    ...
                    {isOpen && (
                      <div className="border-t border-line bg-cream/40 p-5 md:p-6 space-y-4">
                        {s.transcript.map((m, i) => (
                          ...
                          >
                            <div
                              className={`text-[10px] uppercase tracking-[0.2em] mt-1 shrink-0 w-16 ${
                                m.role === "user"
                                  ? "text-ink"
                                  : "text-accent"
                              }`}
                            ...
                            <div className="text-ink-soft whitespace-pre-wrap flex-1">
                              {m.content}
                            </div>
                          </div>
                        ))}
                        {s.feedback && (
                          <div className="mt-5 pt-5 border-t border-line">
                            ...
                            <div className="font-display text-[17px] leading-[1.6] text-ink whitespace-pre-wrap">
                              {s.feedback}
                            </div>
                          </div>
                        )}
